...
export default function KanbanPinned() {
  ...
  useEffect(() => {
    ...
    const channelName = `databases.${DATABASE_ID}.collections.${KANBAN_FOLDER_ID}.documents`;
    ...
    const unsubscribe = client.subscribe(channelName, (response) => {
      ...
      if (isCreate && (payload as any).userId === user.$id) {
        if ((payload as any).pinned) {
          setTasks((prevTasks) => [...prevTasks, payload as any]);
        }
      }
    });
  ...
  }, [user?.$id, DATABASE_ID, KANBAN_FOLDER_ID]);
      ...
      >
        {tasks.map((task, index) => (
          <div
            ...
            onClick={() =>
              router.push(`/kanban/${task.$id}?name=${task?.name}`)
            }
          >
            <label className="sidebar-dropdown-item">{task?.name}</label>
          </div>
        ))}
      </Space>
    </div>
