...
export default function KanbanPinned() {
  ...
  useEffect(() => {
    ...
    const channelName = `databases.${DATABASE_ID}.collections.${NOTES_COLLECTION_ID}.documents`;
    ...
    const unsubscribe = client.subscribe(channelName, (response) => {
      ...
      if (isDelete && (payload as any).userId === user.$id) {
        setNotes((prevNotes) =>
          prevNotes.filter((note) => note.$id !== (payload as any).$id)
        );
      }

      if (isCreate && (payload as any).userId === user.$id) {
        if ((payload as any).pinned) {
          setNotes((prevNotes) => [...prevNotes, payload as any]);
        }
      }
    });
  ...
  }, [user?.$id, DATABASE_ID, NOTES_COLLECTION_ID]);
  return (
